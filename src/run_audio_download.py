...
python run_audio_download.py --year 2018 --month 8 --day 23 --start_hour 5 --end_hour 4 --nextday_end_hour True \
                            ...
                            --ftpurl ftp.atac.com --username sn2018 --ftpfolder VoiceData/ \
...
if __name__ == '__main__':
	...
	@click.option('--password', type = str, prompt = 'FTP password please (input hidden)', hide_input = True, confirmation_prompt = False, help = 'Password for logging on FTP site')
	...
	@click.option('--download_type', default = 'FTP', type = click.Choice(['both', 'FTP','local']), 
	...
	def main(year = 2018,
		...
		if nextday_end_hour:
			print('Downloading Audios from [%s'%yyyymmdd, ' %d'%start_hour, ':00]  to  [%s'%yyyymmdd1, ' %d'%(end_hour+1), ':00]')
		else:
			print('Downloading Audios from [%s'%yyyymmdd, ' %d'%start_hour, ':00]  to  [%s'%yyyymmdd, ' %d'%(end_hour+1), ':00]')
